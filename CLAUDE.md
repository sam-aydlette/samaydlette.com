...
- **It is a real, functioning production system.** Real AWS infrastructure, real CI/CD, real cryptographic signing. Changes must keep it deployable and keep the compliance gate green. Treat `main` as production.
  - **Merging to `main` no longer deploys on its own:** the `deploy` job is bound to the `prod` GitHub Environment, which requires a human reviewer, so every deploy halts before `terraform apply` until someone approves it in the Actions UI. `deploy-with-opa.yml` has **no `schedule:` trigger** — a scheduled deploy could only ever queue at that gate, which is what let twelve days of stale evidence pass unnoticed. Every path to `terraform apply` is now human-initiated *and* human-approved.
  - **One thing does run unattended: `evidence-nightly.yml`.** See the trade recorded under *Security & compliance rules*.
...
- `website/research/` — methodology and scope docs rendered on the site: `authorization-boundary.html`, `the-plumbing.html`. **Read these first for the "why."** Also the long-form essay `staying-in-the-loop.html` (threshold dynamics, titled "Staying in the Loop"). The filename is **not** referenced by `build-ksi-signal.py`, which discovers pages with `rglob("*.html")`; the hardcoded paths live in `tools/essay/*.py` and `tests/test_essay_renumber.py`, so renaming means updating those. `/research/tuning-the-eigenvalue.html` is kept as a redirect stub to the current URL.
...
- `.github/workflows/evidence-nightly.yml` — the unattended nightly VDR refresh: re-scan (Syft + Grype against a fresh vulnerability DB), rebuild the VDR + trend, run the same vulnerability and reconciliation gates, republish five objects under `.well-known/vdr-*`. No `terraform apply`, no website sync. Its alerting is the `alert` job, not the deploy workflow's `notify`.
...
- **Build / verify:** **`make check` is the gate.** Exit 0 means shippable, non-zero means not — read the exit code, not the output. It is hermetic (no AWS, no network, no Terraform state, no generated artifacts), so it runs in any clone. One-time setup: `make dev-setup` (creates `.venv` from `requirements-dev.txt`, which pins exactly what CI installs). Measured on a clean tree, **~4s total**: `lint` (ruff, scripts/+tests/) ~0.04s, `typecheck` (mypy, scripts/) ~0.1s, `test` (pytest tests/, 267 passed / 6 skipped) ~3.0s, `test-policies` (opa fmt + coverage floor + 56 policy tests) ~0.25s. First non-zero wins.
  - `make check-full` adds the **reconciliation gate** (`reconcile`) and `figures-check`. These consume generated artifacts (`ksi-signal.json`, `oscal-ssp.json`, `oscal-poam.json`, `vdr-report.json`) built from `terraform output -json`, so they need AWS and cannot run in a fresh clone; they fail loudly rather than skipping when the artifacts are absent. CI enforces them on every deploy.
  - `test-policies` needs `opa` on PATH at the version pinned in `.github/workflows/deploy-with-opa.yml` (`OPA_VERSION`). An older `opa` cannot parse these Rego v1 policies — and it also silently *skips* ~15 policy tests instead of failing, so a stale binary makes the suite quietly shrink. Check `opa version` if the policy suite looks suspiciously green.
  - ⚠️ **`make pipeline` is not a verification step — it is `dev-setup validate plan deploy` and DEPLOYS TO PRODUCTION.** Never run it to "check" anything.
  - Still required around infra changes: `terraform plan` before any apply, `checkov` per `.checkov.yaml`. Keep a unit test per gate invariant and the committed broken fixture that must fail the gate.
...
- **Keep the bootstrap stack synced with `main`.** `infrastructure/bootstrap/` (the IAM trust layer, CloudFront, DNSSEC, and the remote-state backend) is **operator-applied by hand, not by CI, and holds its own local state** — so its working-tree config drifts from `main` easily, and a `terraform apply` from a stale tree silently **reverts changes already merged via PR**. This has bitten us (a merged `tag:GetResources` grant would be undone by an apply from an out-of-date tree). Rules: (1) whenever a commit touches `infrastructure/bootstrap/`, bring the local tree to `origin/main` for that directory (`git checkout main -- infrastructure/bootstrap/`) so it never lags what's merged; (2) **before any `terraform apply` in `infrastructure/bootstrap/`, sync it to `origin/main` first** and require the `terraform plan` to show *only* your intended change — if it shows unrelated adds/removes, the tree is stale, so stop and sync rather than apply. Never apply bootstrap from a divergent branch. If a merged bootstrap change can't be applied through a synced tree, prefer a surgical, IaC-matching apply (e.g. the exact `aws iam put-role-policy` the merged config specifies) over applying the whole stale stack.
...
- **Published reporting must be under 24 hours old, and something now enforces it.** `scripts/check-evidence-freshness.py` fetches `/.well-known/vdr-report.json` over the public URL and fails if its `emitted_at` is outside the window. Note what the reconciliation gate does *not* do: `check_f_freshness` in `scripts/reconcile.py` only asserts that the staged signal carries the current commit and has an `emitted_at` at all. It has no staleness threshold and never looks at the live edge — which is why twelve days of rot passed with every gate green. Don't mistake one for the other.
- **The unattended-write trade — read this before touching `evidence-nightly.yml`.** That workflow runs nightly with **no required reviewer**, because published evidence has to stay fresh while nobody is awake. It is a genuine unattended production write and it is named as one. What makes it acceptable is its size: it replaces an unattended `terraform apply` over the whole stack with an unattended `PutObject` under a single S3 key prefix. Its dedicated OIDC role (`infrastructure/bootstrap/evidence-nightly.tf`) can write **only** `.well-known/vdr-*`, invalidate CloudFront, and make the read-only calls the reconciliation gate's `--live` sweep needs. It cannot apply Terraform, sync the website, delete an object, or write anywhere else. **The correct response to "the nightly needs one more permission" is to ask whether that work belongs on the unattended path at all — not to widen the role.** The `prod` environment and its reviewer requirement are separate and must stay as they are.
- **The nightly refreshes the VDR only, and binds to the *published* inventory.** It fetches `/.well-known/ksi-signal.json`, verifies its Sigstore bundle against the deploy workflow's pinned identity, and uses that `signal_id` to satisfy reconciliation invariant (e) without rebuilding the inventory. It rebuilds the OSCAL SSP and POA&M **as gate inputs only** — never signed, never published. A changed inventory means changed infrastructure and belongs on the reviewed deploy path. A newly disclosed CVE **cannot** auto-publish: invariants (g) and (h) require a human-authored POA&M entry, so the nightly fails until someone dispositions it. That is the gate working; the `alert` job is what makes the failure visible.
...
- **PR/commit text is public — keep it clean and impersonal.** This repo is public, so every PR title, PR body, and commit message is world-readable. Never paste the chat conversation, a turn-by-turn debugging narrative, or our back-and-forth reasoning. Never include sensitive or account-specific identifiers — KMS key IDs, full ARNs, account IDs, role/secret names, raw IAM policy internals, or verbatim error dumps that carry them. Describe *what changed and why* in outcome terms; reference resources by their Terraform address or role in the system, not by live ID. Verification should state the outcome ("the deploy role's permissions are in place"), not paste the live values you inspected.
- **Don't bury what you need from a human.** If the PR can't fully fix the issue without a human action (a permission you lack, a registrar step, a console-only operation), don't file it as a vague "needs human" line and move on — ask the operator directly and now, with the specific action and identifier they need. The PR itself should be the complete fix for everything you *can* do.
