...
- **Merging to `main` no longer deploys.** The `deploy` job is bound to the `prod` GitHub Environment, which carries a **required reviewer**: a push to `main` runs every check and then *halts* at "Review pending deployment" in the Actions UI until a human approves. Merging is now a second human interrupt alongside the DS-record handoff. The checks themselves (`test`, `compliance-check`/OPA, `security-scan`) are ungated and still run automatically on every push and PR; only the apply/sync/invalidate job waits. The daily `schedule:` run is routed to the unprotected `prod-scheduled` environment so continuous SCA keeps running unattended.
