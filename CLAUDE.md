...
- `scripts/` — the artifact **generators**: `build-ksi-signal.py` (the canonical inventory), `build-oscal-ssp.py`, `build-oscal-poam.py`, and the VDR builder. These are the source of truth for everything published. `scripts/_common.py` holds the helpers they share (`classify` — the inheritance rule four artifacts must agree on — plus `sha256_file`, `prop`, `load_hub`, `sid`, `OSCAL_VERSION`, `FEDRAMP_NS`); it is imported via an `__file__`-relative `sys.path.insert` so every script stays standalone-runnable from any cwd. **`scripts/staged/` is not wired into anything** — parked generators, see its README; never assume a script there runs.
...
- `tools/essay/` — **structural** gates for the long-form pages, run from the repo root: `integrity.py` (HTML balance, duplicate ids, footnote ref/backlink parity, broken anchors), `refcheck.py` (bibliography ↔ citation parity), `renumber.py` (footnote renumbering). `paths.py` is the one place they name a file. The prose tooling that used to live here (`prose_stats.py`, `ed.py`, `cut.py`) is **deleted** — it existed to police AI-written prose and the operator does not use AI to write. Do not reintroduce it, and do not write prose for this site.
...
- **Build / verify:** `make pipeline` is **infrastructure only and publishes nothing** — it is `init → validate → plan → apply-infra` and stops. There is no local publish path: artifact generation, signing, the reconciliation gate and the publish to `/.well-known/` all live in `.github/workflows/deploy-with-opa.yml`, behind the `prod` environment approval. Locally use `make plan` before any apply, `make reconcile` (add `RECONCILE_FLAGS=--live`), `make python-test test-policies gate a11y figures-check`, `make verify-published` to check the live evidence, and `checkov` per `.checkov.yaml`. Keep a unit test per gate invariant and the committed broken fixture that must fail the gate.
