...
# =============================================================================
# A group for trusted third parties who get full read visibility but no write,
# no execute, and cannot read secret VALUES or decrypt CMK-encrypted data.
# Permissions are on the group, not the user (AC-2(1)/AC-6; clears Prowler
# iam_policy_attached_only_to_group_or_roles), same pattern as operators.
# =============================================================================
...
# George — read-only collaborator. The user identity is declared here so it is
# tracked and reproducible; his console password is intentionally NOT managed by
# Terraform (an aws_iam_user_login_profile would store the password in state).
# Set it out-of-band after apply, and George enrolls his own MFA on first login.
resource "aws_iam_user" "george_readonly" {
  name = "george-readonly"
  tags = {
    Purpose   = "read-only-collaborator"
    ManagedBy = "terraform-bootstrap"
  }
}

resource "aws_iam_user_group_membership" "george_readonly" {
  user   = aws_iam_user.george_readonly.name
