...
| POAM-009 | CP-2, CP-7 | CloudFront origin failover not configured | Checkov | CKV_AWS_86 | aws-cloudfront-distribution | N1 | Low | — | No | Open (risk-accepted) |
...
| POAM-029 (Prowler 2026-06-22) | SC-28 | `CKV_AWS_145` / `AVD-AWS-0132` / `s3_bucket_default_encryption` *(Prowler-deprecated)* (samaydlette-com-logs) | The log bucket uses **SSE-S3 (AES256)** by design (`infrastructure/logging.tf`); no control mandates a customer CMK for low-sensitivity access logs about public-facing resources, and a CMK would require granting AWS log-delivery services key access for no confidentiality benefit. | SSE-S3/AES256 at rest on the log bucket — a deliberate choice (SSP SC-28 narrative), same posture as the public website bucket. |
