...
| POAM-033 | Section 508 / WCAG 1.4.3 | pa11y `WCAG2AA...1_4_3.G18.Fail` on `silk-reeling-mirror.html` (footer separators) | The flagged elements are decorative dot separators marked `aria-hidden="true"`; WCAG 1.4.3 exempts pure decoration from contrast minima. The scanner cannot see decorative intent. Suppressed via the gate's exceptions register (`infrastructure/policy/exceptions/data.json`, expires 2027-01-15); the finding stays visible in the gate report under `excepted`. | Accessibility gate (pa11y WCAG 2 AA + OPA decision) over all other page content. |
...
| POAM-034 (Grype 2026-08-13) | SI-2, RA-5 | `GHSA-jmr9-qjv8-65gv` / `CVE-2026-56876` (High — unvalidated symlink path traversal in `extract-zip` 2.0.1), reported by Grype against `sbom-a11y.json` | Not reachable from any deployed artifact, and no upstream fix exists. `extract-zip` is present only in `tools/a11y/` as a transitive dependency of the accessibility scanner (pa11y → puppeteer → @puppeteer/browsers → extract-zip). `tools/a11y` is build tooling: it runs in CI to produce policy input for the deploy gate and is never packaged or deployed — it is inventoried and SBOM-scanned as a build-time component, not because it ships. The package is absent at every depth from the compliance Lambda (`infrastructure/lambda/`), the Lambda zip staging set (`_pkg`), and the Silk Reeling frontend bundle (`_silk_src/frontend`). Separately, there is nothing to upgrade to: the advisory records `firstPatchedVersion: null` and 2.0.1 is `extract-zip`'s final release; `@puppeteer/browsers` 3.x dropped the dependency but is reachable only via puppeteer 25.x, outside the `^24.37.5` range pa11y 9.1.1 declares. Remediation is blocked upstream, not deferred by choice. | The vulnerable code path is never executed: `extract-zip` is reached only when `@puppeteer/browsers` unpacks a downloaded browser archive, and the deploy workflow installs this tree with `PUPPETEER_SKIP_DOWNLOAD=1` and `npm ci --ignore-scripts`, so no browser is fetched and no lifecycle script runs (pa11y renders against the runner's system Chrome). SI-2/RA-5 flaw remediation and vulnerability monitoring are met by continuous SCA over this tree every build (Syft `sbom-a11y.json` + Grype), so a patched release or a change in reachability surfaces automatically. **Revisit condition:** pa11y widening its puppeteer range to a release whose `@puppeteer/browsers` no longer depends on `extract-zip`, or a patched `extract-zip` being published. |
