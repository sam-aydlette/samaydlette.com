...
- **VDR-aligned aggregation:** the build-time VDR aggregator collects findings across the SAST/SCA tools, classifies each detected vulnerability into the FedRAMP Potential Adverse Impact (PAIN) scale (N1–N5 per `VER-EVA-EPA`), determines internet-reachability (IRV per `VER-EVA-EIR`) and likely-exploitability (LEV per `VER-EVA-ELX`), and emits `/.well-known/vdr-report.json`. The build is the report (`VER-RPT-PER`, `VER-TFR-MHR`). The build blocks if any finding exceeds the Class C remediation timeframes (`VDR-TFR-PVR` Class C):
  - N5 + LEV + IRV: 2 days
  - N5 + LEV + NIRV: 4 days
  - N5 + NLEV: 16 days
  - N4 + LEV + IRV: 4 days
...
Vulnerabilities not fully mitigated within 192 days of evaluation are categorized as Accepted (`VER-TFR-MAV`) and tracked in [`poam.md`](../poam.md) with the `VER-RPT-AVI` fields.

KEVs (CISA Known Exploited Vulnerabilities) are remediated according to the BOD 26-04 due dates (`VDR-TFR-KEV`).
