...
DISP = {
    ...
    "sa-11.8": ("implemented", "sp-corporate",
        "Dynamic code analysis runs in CI as a scheduled monthly OWASP ZAP baseline scan (.github/workflows/zap-dast.yml) against the live site, with findings tracked via the ZAP issue and triaged into the POA&M. The scan covers the unauthenticated static surface today; authenticated coverage of the Cognito-gated taiji application is a tracked follow-up. SAST (Checkov, tfsec) and SCA (Syft/Grype) run on every deploy alongside it."),
