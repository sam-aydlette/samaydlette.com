...
POAM_ITEMS = [
    ...
    # Silk Reeling app findings (POAM-019..024) and the Prowler operational item (POAM-025).
    {
        "id": "POAM-019", "controls": ["sc-12", "sc-28"],
        "title": "Secrets Manager automatic rotation not configured",
        "description": "Reclassified false positive (Task 3): CKV2_AWS_57 flags the absence of automatic rotation, but no control mandates that specific mechanism. The basic-auth secret it shared the finding with was removed under Task 3, leaving only the third-party Anthropic API key, which has no programmatic rotation source (AWS cannot mint a new upstream key), so a Secrets Manager rotation Lambda is infeasible by construction. SC-12 rotation is met by documented annual manual rotation, and rotation currency is verified automatically: the runtime KSI emitter reads the secret's LastChangedDate daily and fails an SC-12 validation if rotation lapses past the cadence (395-day grace). Because rotation is met and self-verifying, this is a false positive, not an accepted risk.",
        "weakness_detector_source": "Checkov", "weakness_source_identifier": "CKV2_AWS_57",
        "asset_identifiers": ["aws-secretsmanager-secret::anthropic-api-key"],
        "original_detection_date": "2026-06-08", "status_date": "2026-06-22",
        "original_risk_rating": "low", "adjusted_risk_rating": None, "risk_adjustment": False,
        "status": "false-positive", "category": "false-positive",
    },
    {
        "id": "POAM-020", "controls": ["sa-9", "ca-3"],
        "title": "Interconnection with Anthropic API (non-FedRAMP-authorized external service)",
        "description": "The gated Silk Reeling app Lambda calls the Anthropic API, a non-FedRAMP-authorized external service (SA-9). The only data crossing the authorization boundary is the derived deviation summary (per-joint metrics, scores, hotspots, exercise id) over TLS - no video, raw landmarks, or personal data. SA-9 due diligence rests on Anthropic's commercial API terms (API inputs/outputs not used to train models, retained only transiently for trust-and-safety); this is an assumption re-verified at each annual security review, and a change in those terms reopens this item. Applies only while the app is deployed.",
        "weakness_detector_source": "Architectural decision", "weakness_source_identifier": "silk-reeling-deploy.md",
        "asset_identifiers": ["interconnection::anthropic-api"],
        "point_of_contact": "Sam Aydlette (operator)",
        "resources_required": "Operator time to migrate the feedback call to AWS Bedrock; no net new recurring cost.",
        "remediation_plan": "Migrate the app's feedback generation to Claude on AWS Bedrock (FedRAMP-authorized, in-boundary), which removes the external interconnection entirely.",
        "original_detection_date": "2026-06-08", "scheduled_completion_date": None, "status_date": "2026-06-22",
        "original_risk_rating": "moderate", "adjusted_risk_rating": "low", "risk_adjustment": True,
        "status": "risk-accepted", "category": "interconnection",
    },
    {
        "id": "POAM-021", "controls": ["ia-2.2", "ac-7"],
        "title": "App access via single-factor shared-credential HTTP Basic Auth (no MFA, no lockout)",
        "description": "Closed under Task 3 (2026-06-22): the app's shared-credential HTTP Basic Auth gate was replaced by Amazon Cognito with mandatory TOTP multi-factor authentication and a 14-character password policy, fronted by an API Gateway JWT authorizer. The single-factor shared credential (IA-2(2)) and missing lockout (AC-7) no longer exist.",
        "weakness_detector_source": "Architectural decision", "weakness_source_identifier": "silk-reeling-deploy.md",
        "asset_identifiers": ["silk-reeling::access-control"],
        "remediation_plan": "Replace Basic Auth with Amazon Cognito (mandatory TOTP MFA, strong password policy) plus an API Gateway JWT authorizer on the data routes.",
        "original_detection_date": "2026-06-08", "status_date": "2026-06-22",
        "original_risk_rating": "moderate", "adjusted_risk_rating": "low", "risk_adjustment": True,
        "status": "closed", "category": "closed",
    },
    {
        "id": "POAM-022", "controls": ["ia-2", "ac-3"],
        "title": "API Gateway HTTP API route specifies no authorizer",
        "description": "Closed under Task 3 (2026-06-22): the API Gateway HTTP API now carries a Cognito JWT authorizer on the ANY /api/{proxy+} routes, so an unauthenticated request to the data API is rejected at the gateway (401) before the Lambda runs. The $default route stays open only so the SPA shell can load. CKV_AWS_309 passes.",
        "weakness_detector_source": "Checkov", "weakness_source_identifier": "CKV_AWS_309",
        "asset_identifiers": ["aws-apigatewayv2::silk-reeling"],
        "original_detection_date": "2026-06-08", "status_date": "2026-06-22",
        "original_risk_rating": "moderate", "adjusted_risk_rating": "low", "risk_adjustment": True,
        "status": "closed", "category": "closed",
    },
    {
        "id": "POAM-023", "controls": ["ac-7", "sc-5"],
        "title": "No brute-force / rate-limit protection on the Basic Auth endpoint",
        "description": "Closed under Task 3 (2026-06-22): API Gateway usage-plan throttling (20 req/s, burst 10) now fronts the app and Cognito provides account lockout, so credential guessing against the endpoint is throttled and locked out. The unthrottled Basic Auth endpoint no longer exists.",
        "weakness_detector_source": "Security review", "weakness_source_identifier": "security-review.md",
        "asset_identifiers": ["silk-reeling::access-control"],
        "original_detection_date": "2026-06-08", "status_date": "2026-06-22",
        "original_risk_rating": "moderate", "adjusted_risk_rating": "low", "risk_adjustment": True,
        "status": "closed", "category": "closed",
    ...
    {
        "id": "POAM-024", "controls": ["au-2", "au-3"],
        "title": "API Gateway HTTP API access logging not enabled",
        "description": "Closed under Task 7 (2026-06-18): the API Gateway stage now has access logging configured to a CloudWatch log group via the vended-logs delivery path (AccessLogSettings). CKV_AWS_76 passes.",
        "weakness_detector_source": "Checkov", "weakness_source_identifier": "CKV_AWS_76",
        "asset_identifiers": ["aws-apigatewayv2::silk-reeling"],
        "original_detection_date": "2026-06-08", "status_date": "2026-06-18",
        "original_risk_rating": "low", "adjusted_risk_rating": None, "risk_adjustment": False,
        "status": "closed", "category": "closed",
    },
    {
        "id": "POAM-025", "controls": ["ia-2.1", "ia-2.2", "ia-2.8", "cm-6"],
        "title": "Root and operator MFA is virtual TOTP, not hardware/phishing-resistant",
        "description": "The root account and the operator IAM user (saydlette-dev) both authenticate with virtual authenticator-app (TOTP) MFA rather than a hardware/phishing-resistant authenticator. Multi-factor authentication (IA-2(1)/(2)) is met - MFA is enabled on both - but the stricter phishing-resistant objective (IA-2(8): hardware token or passkey) is not. Surfaced by the 2026-06-22 Prowler scan. Compensating control: virtual TOTP MFA is enabled on both principals now.",
        "weakness_detector_source": "Prowler", "weakness_source_identifier": "root-account-hardware-mfa-enabled; iam_user_hardware_mfa_enabled",
        "asset_identifiers": ["aws-account::root", "aws-iam-user::saydlette-dev"],
        "point_of_contact": "Sam Aydlette (operator)",
        "resources_required": "A FIDO2/WebAuthn hardware security key (~$25-50); operator time to enroll.",
        "remediation_plan": "Enroll a hardware security key (FIDO2/WebAuthn) as the MFA device for the root account and the operator IAM user, satisfying the phishing-resistant objective (IA-2(8)).",
        "original_detection_date": "2026-06-22", "scheduled_completion_date": "TBD (pending hardware authenticator procurement)", "status_date": "2026-06-22",
        "original_risk_rating": "moderate", "adjusted_risk_rating": "low", "risk_adjustment": True,
